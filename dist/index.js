...
class LLRP extends events_1.EventEmitter {
  ...
  connect() {
    ...
    this.socket.setTimeout(60000, () => {
      if (this.connected) {
        ...
        process.nextTick(() => {
          this.connected = false;
          this.emit(
            llrp_1.RfidReaderEvent.Timeout,
    ...
    this.client.on('end', () => {
      ...
      process.nextTick(() => {
        this.connected = false;
        this.emit(
          llrp_1.RfidReaderEvent.Disconnect,
  ...
  disconnect() {
    ...
    this.sendMessage(
      this.client,
      getLlrpMessage_1.GetLlrpMessage.deleteRoSpec(defaultRoSpecId)
    );
  ...
  disableRFTransmitter() {
    ...
    this.sendMessage(
      this.client,
      getLlrpMessage_1.GetLlrpMessage.disableRoSpec(defaultRoSpecId)
    );
    this.resetIsStartROSpecSent();
    return true;
  }
      ...
      for (const index in messagesKeyValue) {
        ...
        switch (message.getType()) {
          ...
          case messagesType_1.MessagesType.ENABLE_ROSPEC_RESPONSE:
            ...
            this.emit('REPONSE_ENABLE_RO_SPEC');
            break;
          ...
          case messagesType_1.MessagesType.DELETE_ROSPEC_RESPONSE:
            if (!this.allReaderRospecDeleted) {
              ...
              this.sendMessage(
                this.client,
                getLlrpMessage_1.GetLlrpMessage.closeConnection()
              );
            }
            break;
          case messagesType_1.MessagesType.START_ROSPEC_RESPONSE:
            ...
            this.sendMessage(
              this.client,
              getLlrpMessage_1.GetLlrpMessage.enableEventsAndReport()
            );
            break;
          ...
          case messagesType_1.MessagesType.KEEPALIVE:
            this.sendMessage(
              this.client,
              getLlrpMessage_1.GetLlrpMessage.keepAliveAck()
            );
            break;
          default:
  ...
  checkErrorInResponse(message) {
    ...
    param.forEach((decodedParameters) => {
      ...
      if (decodedParameters.type === parametersConstants_1.default.LLRPStatus) {
        ...
        if (statusCode) {
          const errorDescriptionByteCount =
            decodedParameters.value.readInt16BE(2);
          const errorDescriptionBuffer = Buffer.allocUnsafe(
            errorDescriptionByteCount
          );
          decodedParameters.value.copy(
            errorDescriptionBuffer,
            0,
            4,
            errorDescriptionByteCount + 4
          );
          const errorDescription = `${errorDescriptionBuffer.toString(
            'utf8'
          )} in ${message.getTypeName()}`;
          this.emit(
            llrp_1.RfidReaderEvent.LlrpError,
            new Error(`${statusCode}: ${errorDescription}`)
          );
        }
  ...
  handleReaderNotification(message) {
    ...
    parametersKeyValue.forEach((decodedParameters) => {
      if (
        ...
        parametersConstants_1.default.ReaderEventNotificationData
      ) {
        ...
        if (subParameters[parametersConstants_1.default.ROSpecEvent]) {
          ...
          if (
            subParameters[parametersConstants_1.default.ROSpecEvent].readUInt8(
              0
            ) === 1
          ) {
    ...
    if (!this.isReaderConfigReset) {
      // reset them.
      this.client.write(
        getLlrpMessage_1.GetLlrpMessage.resetConfigurationToFactoryDefaults()
      );
  ...
  handleReaderConfiguration() {
    if (!this.allReaderAccessSpecDeleted) {
      this.sendMessage(
        this.client,
        getLlrpMessage_1.GetLlrpMessage.deleteAllAccessSpec()
      );
      this.allReaderAccessSpecDeleted = true;
    } else if (!this.allReaderRospecDeleted) {
      this.sendMessage(
        this.client,
        getLlrpMessage_1.GetLlrpMessage.deleteAllROSpecs()
      );
    } else if (!this.isExtensionsEnabled) {
      // enable extensions for impinj reader
      this.sendMessage(
        this.client,
        getLlrpMessage_1.GetLlrpMessage.enableExtensions()
      );
      ...
      this.sendMessage(
        this.client,
        getLlrpMessage_1.GetLlrpMessage.setReaderConfig()
      ); // send SET_READER_CONFIG, global reader configuration in reading tags.
      this.isReaderConfigSet = true; // we have set the reader configuration.
    } else {
      this.sendMessage(
        this.client,
        getLlrpMessage_1.GetLlrpMessage.addRoSpec(
          defaultRoSpecId,
          this.radioOperationConfig
        )
      );
  ...
  handleROAccessReport(message) {
    process.nextTick(() => {
      ...
      const parametersKeyValue = decode_1.decodeParameter(
      ...
      if (parametersKeyValue) {
        parametersKeyValue.forEach((decodedParameters) => {
          ...
          if (
            decodedParameters.type ===
            parametersConstants_1.default.TagReportData
          ) {
            const tag = {};
            const subParameters = this.mapSubParameters(decodedParameters);
            if (subParameters[parametersConstants_1.default.EPC96]) {
              tag.EPC96 =
                subParameters[parametersConstants_1.default.EPC96].toString(
                  'hex'
                );
            }
            if (subParameters[parametersConstants_1.default.EPCData]) {
              tag.EPCData =
                subParameters[parametersConstants_1.default.EPCData].toString(
                  'hex'
                );
            }
            if (subParameters[parametersConstants_1.default.AntennaID]) {
              tag.antennaID =
                subParameters[
                  parametersConstants_1.default.AntennaID
                ].readUInt16BE(0);
            }
            if (subParameters[parametersConstants_1.default.TagSeenCount]) {
              tag.tagSeenCount =
                subParameters[
                  parametersConstants_1.default.TagSeenCount
                ].readUInt16BE(0);
            }
            if (subParameters[parametersConstants_1.default.PeakRSSI]) {
              tag.peakRSSI =
                subParameters[parametersConstants_1.default.PeakRSSI].readInt8(
                  0
                );
            }
            if (subParameters[parametersConstants_1.default.ROSpecID]) {
              tag.roSpecID =
                subParameters[
                  parametersConstants_1.default.ROSpecID
                ].readUInt32BE(0);
            }
            if (subParameters[parametersConstants_1.default.SpecIndex]) {
              tag.specIndex =
                subParameters[
                  parametersConstants_1.default.SpecIndex
                ].readUInt16BE(0);
            }
            if (
              subParameters[
                parametersConstants_1.default.InventoryParameterSpecID
              ]
            ) {
              tag.inventoryParameterSpecID =
                subParameters[
                  parametersConstants_1.default.InventoryParameterSpecID
                ].readUInt16BE(0);
            }
            if (subParameters[parametersConstants_1.default.ChannelIndex]) {
              tag.channelIndex =
                subParameters[
                  parametersConstants_1.default.ChannelIndex
                ].readUInt16BE(0);
            }
            if (subParameters[parametersConstants_1.default.C1G2PC]) {
              tag.C1G2PC =
                subParameters[
                  parametersConstants_1.default.C1G2PC
                ].readUInt16BE(0);
            }
            if (subParameters[parametersConstants_1.default.C1G2CRC]) {
              tag.C1G2CRC =
                subParameters[
                  parametersConstants_1.default.C1G2CRC
                ].readUInt16BE(0);
            }
            if (subParameters[parametersConstants_1.default.AccessSpecID]) {
              tag.accessSpecID =
                subParameters[
                  parametersConstants_1.default.AccessSpecID
                ].readUInt32BE(0);
            }
            if (
              subParameters[parametersConstants_1.default.FirstSeenTimestampUTC]
            ) {
              // Note: Here is losing precision because JS numbers are defined to be double floats
              const firstSeenTimestampUTCus = new Int64(
                subParameters[
                  parametersConstants_1.default.FirstSeenTimestampUTC
                ],
                0
              );
              tag.firstSeenTimestampUTC =
                firstSeenTimestampUTCus.toNumber(true); // microseconds
            }
            if (
              subParameters[parametersConstants_1.default.LastSeenTimestampUTC]
            ) {
              // Note: Here is losing precision because JS numbers are defined to be double floats
              const lastSeenTimestampUTCus = new Int64(
                subParameters[
                  parametersConstants_1.default.LastSeenTimestampUTC
                ],
                0
              );
            ...
            if (subParameters[parametersConstants_1.default.Custom]) {
              tag.custom =
                subParameters[parametersConstants_1.default.Custom].toString(
                  'hex'
                );
              if (
                this.radioOperationConfig.enableReadingTid &&
                ...
                const impinjParameterSubtype =
                  subParameters[
                    parametersConstants_1.default.Custom
                  ].readUInt32BE(4);
                switch (impinjParameterSubtype) {
                  case parameters_1.CustomParameterSubType
                    .IMPINJ_SERIALIZED_TID:
  ...
  sendEnableRospec(sendTwoTimes) {
    ...
    this.sendMessage(
      this.client,
      getLlrpMessage_1.GetLlrpMessage.enableRoSpec(defaultRoSpecId)
    );
  }
  /**
  ...
  sendStartROSpec() {
    ...
    if (!this.isStartROSpecSent) {
      ...
      this.sendMessage(
        this.client,
        getLlrpMessage_1.GetLlrpMessage.startRoSpec(defaultRoSpecId)
      );
    }
  }
  /**
